...
// TestParserValueNestingLimit ensures deeply nested arrays and inline tables
// are rejected with a ParserError rather than overflowing the goroutine stack,
// which would be an unrecoverable fatal error terminating the whole process.
func TestParserValueNestingLimit(t *testing.T) {
	examples := []struct {
		desc  string
		input string
	}{
		{
			desc:  "arrays",
			input: "a=" + strings.Repeat("[", maxValueNesting+1) + strings.Repeat("]", maxValueNesting+1),
		},
		{
			desc:  "inline tables",
			input: "a=" + strings.Repeat("{k=", maxValueNesting+1) + "1" + strings.Repeat("}", maxValueNesting+1),
		},
	}
	...
	for _, e := range examples {
		t.Run(e.desc, func(t *testing.T) {
			...
			err := p.Error()
			if err == nil {
				t.Fatal("expected parse error for over-nested value")
			}
...
// TestParserValueNestingAtLimit ensures documents nested exactly to the limit
// still parse successfully: the guard must not reject valid input.
func TestParserValueNestingAtLimit(t *testing.T) {
	input := "a=" + strings.Repeat("[", maxValueNesting) + strings.Repeat("]", maxValueNesting)
	...
	if err := p.Error(); err != nil {
		t.Fatalf("nesting at the limit must parse, got: %v", err)
	}
}
